...
int main() {
    ...
    for (int i = 0; i < no_of_lines; i++) {
        ...
        for (size_t index = 0; index < line.length();) {
            if (line.compare(index, string("do_mul()").length(), "do_mul()") ==
                0) {
                mul_start = true;
                index += string("do_mul()").length();
            } else if (line.compare(index, string("don't_mul()").length(),
                                    "don't_mul()") == 0) {
                mul_start = false;
                index += string("don't_mul()").length();
            } else if (line.compare(index, string("mul(").length(), "mul(") ==
                       0) {
                        ...
                        if (comma != std::string::npos) {
                            ...
                            if (is_number(a) && is_number(b)) {
                                try {
                                    ...
                                    // This block executes if stoi() throws an
                                    // exception
                                }
                                index = end + 1;
                            }
            ...
            } else if (line.compare(index, string("do_add()").length(),
                                    "do_add()") == 0) {
                add_start = true;
                index += string("do_add()").length();
            } else if (line.compare(index, string("don't_add()").length(),
                                    "don't_add()") == 0) {
                add_start = false;
                index += string("don't_add()").length();
            } else if (line.compare(index, string("add(").length(), "add(") ==
                       0) {
